...
export class GroupingConfigDialogComponent {
  ...
  constructor(
    ...
    this.groupingColumns = dialogData.groupingColumns.map(gpCol => columnByName[gpCol.columnName]);
   ...
   */
  private calculateRemainingColumns(allColumns: ColumnMetaDataDef[], groupingColumns: ColumnMetaDataDef[]) {
  ...
  validate() {
    this.dialogRef.close(this.groupingColumns.map((col): GroupingColumn => ({
      columnName: col.name
    })))
  }
}
